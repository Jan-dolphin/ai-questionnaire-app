...
interface Props {
  value: string; // The JSON string of options from db, e.g. "{\"A\":\"Option 1\", \"B\":\"Option 2\"}"
  onChange: (newValue: string) => void;
}

export function SingleChoiceOptions({ value, onChange }: Props) {
  ...
  const handleAddOption = () => {
    // Determine next letter key (A, B, C...)
    const currentKeys = Object.keys(options);
    const nextKey = String.fromCharCode(65 + currentKeys.length); // 65 is 'A'
  ...
  return (
    <div style={{ marginTop: '16px', padding: '16px', backgroundColor: 'var(--background)', borderRadius: '8px', border: '1px solid var(--border)' }}>
      <h4 style={{ marginBottom: '12px', fontSize: '0.9rem', color: 'var(--text-muted)' }}>Vizuální editor možností (Jedna odpověď)</h4>
